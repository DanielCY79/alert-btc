...
/**
 * 实时监控任务调度入口。
 * 负责定时加载目标交易对，并把实际处理工作交给 {@link PriceActionExecutionProcessor}。
 */
@Service
...
public class AlertService {
    ...
    private final StrategyExecutionProcessor strategyExecutionProcessor;

    public AlertService(StrategyExecutionProcessor strategyExecutionProcessor) {
        this.strategyExecutionProcessor = strategyExecutionProcessor;
    }
     ...
     */
    @Scheduled(
            fixedRateString = "${monitoring.cycle:15000}",
    ...
    public void monitoring() {
        ...
        try {
            strategyExecutionProcessor.process(targetSymbol);
        } catch (Exception e) {
