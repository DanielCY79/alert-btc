...
import com.mobai.alert.strategy.priceaction.runtime.BreakoutRecord;
...
public class PriceActionExecutionProcessor implements StrategyExecutionProcessor {

    private static final Logger log = LoggerFactory.getLogger(PriceActionExecutionProcessor.class);
    ...
    @Value("${monitoring.kline.limit:360}")
    private int klineLimit;

    private String contextInterval = MultiTimeframeDefaults.CONTEXT_INTERVAL;

    private int contextKlineLimit = MultiTimeframeDefaults.CONTEXT_KLINE_LIMIT;

    private boolean requireExecutionContext = MultiTimeframeDefaults.REQUIRE_EXECUTION_CONTEXT;

    private boolean executionContextWarmupEnabled = MultiTimeframeDefaults.EXECUTION_CONTEXT_WARMUP_ENABLED;
    ...
    public PriceActionExecutionProcessor(BinanceApi binanceApi,
                                         PriceActionSignalEvaluator alertRuleEvaluator,
    ...
    private FeatureSnapshot buildExecutionSnapshot(String symbol, List<BinanceKlineDTO> executionKlines) {
        ...
        marketFeatureSnapshotService.rememberLatestSnapshot(strategyMetadata.id(), executionSnapshot);
    ...
    private FeatureSnapshot loadContextSnapshot(String symbol) {
        ...
        marketFeatureSnapshotService.rememberLatestSnapshot(strategyMetadata.id(), contextSnapshot);
    ...
    private FeatureSnapshot tryReuseWarmupContextSnapshot(String symbol) {
        ...
        FeatureSnapshot cachedSnapshot = marketFeatureSnapshotService.getLatestSnapshot(strategyMetadata.id(), symbol, contextInterval);
    ...
    private String stateKey(String symbol, String interval) {
        return strategyMetadata.namespace(symbol + "@" + interval);
    }
