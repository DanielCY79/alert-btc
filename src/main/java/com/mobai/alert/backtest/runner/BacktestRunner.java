...
import com.mobai.alert.backtest.BacktestService;
import com.mobai.alert.backtest.export.BacktestExcelExportService;
...
public class BacktestRunner implements ApplicationRunner {
    ...
    public BacktestRunner(BacktestService backtestService,
    ...
    public void run(ApplicationArguments args) {
        ...
        BatchBacktestResult result = backtestService.runDefaultBacktestBatch();
        backtestExcelExportService.exportIfEnabled(result).ifPresent(outcome ->
                log.info("Backtest capital summary: rawFinalEquity={} USDT, policyFinalEquity={} USDT, policyReturn={}%, policyMaxDrawdown={} USDT ({}%), workbook={}",
                        ...
                        outcome.outputPath()));
        log.info("Backtest job finished:\n{}", backtestService.formatBatchResult(result));
